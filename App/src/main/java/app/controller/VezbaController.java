...
public class VezbaController {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<?> obrisiVezbu(@PathVariable("id") Long id){
        try{
            vezbaService.obrisiVezbu(id);
            return ResponseEntity.ok().body("Vezba je uspesno obrisana.");
        }catch(Exception e){
            return ResponseEntity.badRequest().body("Greska pri brisanju vezbe: " + e.getMessage());
        }
    }
}
